...
RELS = json.load(io.open(os.path.join(DATA, 'relations.json'), encoding='utf-8'))
REL = RELS['resources']; VEXTRA = RELS.get('video_extra', {})
def rel_tuts(r):
    x = REL.get(r['slug'], {}); st = [v for v in x.get('st', []) if v in by_vid]; rt = [v for v in x.get('rt', []) if v in by_vid]
    return st or rt[:1]
def rel_res(r):
    x = REL.get(r['slug'], {}); sr = [z for z in x.get('sr', []) if z in by_slug]; rr = [z for z in x.get('rr', []) if z in by_slug]
    return (sr or rr[:2])[:3]
def vid_res(v):
    st = [z for z, x in REL.items() if v['id'] in x.get('st', []) and z in by_slug]
    rt = [z for z, x in REL.items() if v['id'] in x.get('rt', []) and z in by_slug] + [z for z in VEXTRA.get(v['id'], {}).get('rr', []) if z in by_slug]
    return (st or list(dict.fromkeys(rt))[:2])[:3]
for _v in VIDS: _v['related_resources'] = vid_res(_v)
for _r in RES: _r['related_tutorials'] = rel_tuts(_r); _r['related_resources'] = rel_res(_r)
...
def md_to_html(md, slug=''):
    ...
    while i < len(lines):
        ...
        if l.startswith(':::figure'):
            ...
            out.append(f'<div class="fig-ph"><b>Imagen en preparación</b><span>{e(cap) or "Muy pronto verás aquí la captura de este paso"}</span></div>'); i += 1; continue
        if l.startswith(':::file'):
            out.append(f'<div class="fig-ph"><b>Archivo descargable</b><span>{e(l[len(":::file"):].strip())} · disponible pronto</span></div>'); i += 1; continue
        ...
        if l.startswith(':::embed'):
            ...
            else: out.append('<div class="fig-ph"><b>Video de este paso</b><span>Disponible pronto</span></div>')
            i += 1; continue
        if l == '---':
